...
void StallGuardResultSensor::update() {
  if (this->parent_->is_failed()) return;
  this->publish_state(this->parent_->read_register(SG_RESULT));
}

void MotorLoadSensor::dump_config() { LOG_SENSOR(" ", "TMC2209 Motor Load Sensor", this); }
void MotorLoadSensor::update() {
  if (this->parent_->is_failed()) return;
  this->publish_state(this->parent_->get_motor_load() * 100.0f);
}
...
void PWMScaleSumSensor::update() {
  if (this->parent_->is_failed()) return;
  this->publish_state(this->parent_->read_field(PWM_SCALE_SUM_FIELD));
}

void PWMScaleAutoSensor::dump_config() { LOG_SENSOR(" ", "TMC2209 PWM Scale Auto Sensor", this); }
void PWMScaleAutoSensor::update() {
  if (this->parent_->is_failed()) return;
  this->publish_state(this->parent_->read_field(PWM_SCALE_AUTO_FIELD));
}

void PWMOFSAutoSensor::dump_config() { LOG_SENSOR(" ", "TMC2209 OFS Auto Sensor", this); }
void PWMOFSAutoSensor::update() {
  if (this->parent_->is_failed()) return;
  this->publish_state(this->parent_->read_field(PWM_OFS_AUTO_FIELD));
}

void PWMGradAutoSensor::dump_config() { LOG_SENSOR(" ", "TMC2209 Grad Auto Sensor", this); }
void PWMGradAutoSensor::update() {
  if (this->parent_->is_failed()) return;
  this->publish_state(this->parent_->read_field(PWM_GRAD_AUTO_FIELD));
}
