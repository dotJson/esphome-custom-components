...
struct IndexPulseStore {
  ...
  static void IRAM_ATTR HOT pulse_isr(IndexPulseStore *arg) {
    (*(arg->current_position_ptr)) +=
        (int8_t) (*(arg->direction_ptr));
  }
};
...
class TMC2209Stepper : public TMC2209Component, public Stepper {
 public:
  ...
  void set_control_method(ControlMethod method) {
 ...
 protected:
  ...
  // Explicit initialization avoids an undefined direction state
  // before the first commanded move.
  Direction direction_{Direction::STANDSTILL};
  /**/
};
